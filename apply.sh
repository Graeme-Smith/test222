...
original_name="test222"
original_urlname="test222"
original_description="Awesome test222 created by Graeme-Smith" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/test222 "${name}"
...
then
    ...
    echo "then run 'test222' or 'python -m test222'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'test222' or 'python -m test222' or 'poetry run test222'"
fi
